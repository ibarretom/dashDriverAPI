...
describe('Most visited address controller', () => {
  beforeAll(async () => {
    ...
    await request(app)
      ...
      .send({
        ...
        iso_date: new Date(2023, 2, 3, 12, 0, 0).toISOString() /* 12 horas */,
      })
      .set({
        Authorization: `Bearer ${token}`,
      })
  ...
  it('Should be able to list all addresses most visited', async () => {
    const address_response = await request(app)
      .get('/address/mostVisited')
      .set({
        Authorization: `Bearer ${token}`,
      })
    ...
    expect(address.all_time).toHaveLength(3)
    expect(address.all_time[0].id).toBe('Barreto - Niterói')
    expect(address.all_time[0].value).toBe(2)

    expect(address.morning).toHaveLength(1)
    expect(address.afternoon).toHaveLength(1)
    expect(address.night).toHaveLength(1)
    expect(address.dawn).toHaveLength(1)
  })
})
